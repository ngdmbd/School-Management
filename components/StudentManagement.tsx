...
import { Student, Translation, InstitutionType, Language } from '../types';
...
const StudentManagement: React.FC<StudentManagementProps> = ({ students, setStudents, t, lang }) => {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">{t.rollNo}</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">{t.studentName}</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">{t.class}</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">{t.grade}</th>
                  ...
                  <tr className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded text-xs font-bold ${
                        ...
                        student.grade.startsWith('B') ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'
                      }`}>
                        {student.grade}
                      </span>
                    </td>
              ...
              {filteredStudents.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-slate-400">
                    {lang === 'bn' ? 'কোন তথ্য পাওয়া যায়নি' : 'No students found'}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              ...
              <button onClick={closeForm} className="text-slate-400 hover:text-slate-600 text-2xl">&times;</button>
            </div>
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              <div>
                ...
                <div className="flex space-x-2">
                  <button 
                    onClick={() => setFormData({...formData, institutionType: 'school', class: CLASSES_SCHOOL[0]})}
                    className={`flex-1 py-2 rounded-lg border ${formData.institutionType === 'school' ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'border-slate-200'}`}
                  >
                    {t.schoolOption}
                  </button>
                  <button 
                    onClick={() => setFormData({...formData, institutionType: 'madrasa', class: CLASSES_MADRASA[0]})}
                    className={`flex-1 py-2 rounded-lg border ${formData.institutionType === 'madrasa' ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'border-slate-200'}`}
                  >
                    {t.madrasaOption}
                  </button>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">{t.rollNo}</label>
                  <input
                    type="text"
                    className="w-full px-4 py-2 rounded-lg border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none"
                    value={formData.roll}
                ...
                <div>
                  ...
                  <select
                    className="w-full px-4 py-2 rounded-lg border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none bg-white"
                    value={formData.class}
                    onChange={e => setFormData({...formData, class: e.target.value})}
                  >
                    {(formData.institutionType === 'school' ? CLASSES_SCHOOL : CLASSES_MADRASA).map(c => (
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex space-x-3">
              <button 
                onClick={closeForm}
                className="flex-1 px-4 py-2 border border-slate-200 rounded-lg text-slate-600 font-medium hover:bg-white"
              >
                {t.cancel}
              </button>
              <button 
